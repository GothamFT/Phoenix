...
void sendEmail(Result res, Parameters params) {
  ...
  oss << "sendemail -f " << params.senderAddress << " -t " << params.receiverAddress << " -u \" Trade " << res.id <<" (";

  if (res.totPerf() >= 0) {
    oss << "+" << res.totPerf() * 100.0;
  }
  else {
    oss << res.totPerf() * 100.0;
  }
  ...
  oss << "      <caption style=\\\"" << captionStyle << "\\\"> Trade " << res.id << "</caption>";
